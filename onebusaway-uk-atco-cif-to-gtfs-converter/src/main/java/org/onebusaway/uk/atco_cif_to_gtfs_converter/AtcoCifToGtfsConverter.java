...
public class AtcoCifToGtfsConverter {
	...
	private static Logger _log = LoggerFactory
			.getLogger(AtcoCifToGtfsConverter.class);
	...
	public void run() throws IOException {
		...
		if (_prunedStopsWithNoLocationInfoCount > 0) {
			_log.info(String
					.format("pruned stops with no location info: %d stops used by %d stop times in %d trips",
							_prunedStopsWithNoLocationInfoCount,
							_prunedStopTimesCount, _prunedTripsCount));
		}
	}
	...
	private void constructTrips() {
		for (List<JourneyHeaderElement> journies : _journeysById.values()) {
			for (int i = 0; i < journies.size(); ++i) {
				...
				RouteDescriptionElement routeDescription = metadata
						.getRouteDescriptionForDirection(journey
								.getRouteDirection());
				if (routeDescription != null) {
					trip.setTripHeadsign(routeDescription.getRouteDescription());
				}
				Integer directionId = metadata
						.getDirectionIdForDirection(journey.getRouteDirection());
				if (directionId != null) {
	...
	private Route getRouteForJourney(JourneyHeaderElement journey) {
		...
		if (route == null) {
			...
			if (metaData != null) {
				...
				if (desc != null && desc.getRouteDescription() != null && !desc.getRouteDescription().isEmpty()){
	...
	private int getRouteTypeForJourney(JourneyHeaderElement journey) {
		...
		VehicleTypeElement vehicleTypeElement = _vehicleTypesById.get(journey
				.getVehicleType());
	...
	private AgencyAndId getServiceIdForJourney(JourneyHeaderElement journey) {
		...
		if (calendar == null) {
			...
			for (JourneyDateRunningElement modification : journey
					.getCalendarModifications()) {
				...
				int exceptionType = modification.getOperationCode() == 1 ? 1
						: 2;
	...
	private ServiceDate getServiceDate(Date date) {
		Calendar c = Calendar
				.getInstance(TimeZone.getTimeZone("Europe/London"));
		c.setTime(date);
		return new ServiceDate(c);
	}

	private AgencyAndId constructServiceIdForJourney(
			JourneyHeaderElement journey) {
	...
	private String getServiceDateModificationsSuffix(
			JourneyHeaderElement journey) {
		List<JourneyDateRunningElement> modifications = journey
				.getCalendarModifications();
		...
		if (suffix == null) {
			...
			suffix = format
					.format(_serviceDateModificationSuffixByKey.size() + 1);
	...
	private void constructTimepoints(JourneyHeaderElement journey, Trip trip) {
		...
		for (JourneyTimePointElement timePoint : journey.getTimePoints()) {
			...
			/**
			 * A NULL stop indicates a stop that has been pruned because it
			 * doesn't have location information. We do not produce stop times
			 * for these stops.
			 */
			if (stop == null) {
			...
			if (timePoint.getArrivalTime() != 0
					|| timePoint.getDepartureTime() != 0 || first) {
	...
	private Stop findStop(String stopId) {
		...
		GreaterManchesterTimetableRowListElement rowList = _greaterManchesterRowListsByLocationId
				.get(stopId);
		if (rowList != null) {
	...
	private LocationElement getLocationForId(String stopId) {
		...
		/**
		 * I've noticed a strange case where a journey references a stop with an
		 * id "blahX" when only a stop with id "blah" exists.
		 */
		if (location == null) {
			if (stopId.length() > 1) {
	...
	private Stop getAtcoStop(LocationElement location) {
		...
		Stop stop = _dao.getStopForId(id);
		if (stop == null) {
			AdditionalLocationElement additionalLocation = _additionalLocationById
					.get(locationId);
			if (additionalLocation == null) {
				throw new AtcoCifException("found location with id="
						+ locationId
			...
			NationalExpressLocationNameElement nxNameElement = _nxLocationNamesById
					.get(locationId);
	...
	private void writeGtfs() throws IOException {
		...
		schemaFactory.addFactory(GtfsEntitySchemaFactory
	...
	private class HandlerImpl implements ContentHandler {
		...
		public void startElement(Element element) {
			if (element instanceof JourneyHeaderElement) {
				...
				AgencyAndId journeyId = new AgencyAndId(
						journey.getOperatorId(), journey.getJourneyIdentifier());
				List<JourneyHeaderElement> journies = _journeysById
						.get(journeyId);
				if (journies == null) {
				...
				OperatorElement existing = _operatorsById.put(
				...
				_nxLocationNamesById.put(nxNameElement.getLocationId(),
						nxNameElement);
